...
This is a Rails 7.2 monolith, not an API application:

- Ruby is pinned to `4.0.5` in `.ruby-version`; Rails is `~> 7.2.0`.
...
- Match the repository's Ruby style: simple classes, explicit transactions, keyword metadata arguments, and Minitest `test "..."` cases. Avoid introducing APIs that Rails 7.2 or Ruby 4.0 do not support.
- `config.load_defaults` is 7.2. In particular, Rails defers Active Job enqueues made in Active Record transactions until commit; Que jobs continue to use their existing `enqueue` API.
